...
interface ContactsListProps {
  users: User[];
  setChosenPlayer: React.Dispatch<React.SetStateAction<User>>;
}


const ContactsList : React.FC<ContactsListProps> = ({ users, setChosenPlayer}) => {
  return (
    <ul
      role="list"
          ...
          <div className="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
            ...
            {user.isOnline ? (
              ...
              <p className="mt-1 text-xs leading-5 dark:text-gray-200 text-gray-500">
                Last seen: <time dateTime={user.lastSeen?.toLocaleTimeString()}>{user.lastSeen?.toLocaleDateString()}</time>
              </p>
            )}
          </div>
        </li>
